...
WHAT IT DOES:
  ...
  2. Uses Gemini's embedding model to convert text → vectors (lists of numbers)
...
import os
# Import Gemini's embedding model
...
# Import configuration (ensures GEMINI_API_KEY is available)
import config  # noqa: F401 — ensures GEMINI_API_KEY is set in os.environ
...
def create_vector_store() -> FAISS:
    ...
    HOW IT WORKS:
      Step 1: Create a GoogleGenerativeAIEmbeddings object
        - This loads Gemini's "gemini-embedding-001" model
    ...
    # This calls Gemini's API to embed each document
